...
void GBNRdtReceiver::receive(const Packet &packet) {
    ...
    if (checkSum == packet.checksum && this->expectSequenceNumberRcvd == packet.seqnum) {
        ...
        if (checkSum != packet.checksum) {
            if (!TESTING) {pUtils->printPacket("接收方没有正确收到发送方的报文,数据校验错误", packet);}
        } else {
            if (!TESTING) {pUtils->printPacket("接收方没有正确收到发送方的报文,报文序号不对", packet);}
        }
        if(!TESTING) {pUtils->printPacket("接收方重新发送上次的确认报文", lastAckPkt);}
